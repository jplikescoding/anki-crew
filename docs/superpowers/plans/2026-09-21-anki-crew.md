...
**Interfaces:**
- Consumes: nothing.
- Produces: `build_db(path) -> sqlite3.Connection`, `add_deck(con, did, name)`, `add_note(con, nid, mid, fields: list[str])`, `add_card(con, cid, nid, did, ivl=0)`, `add_review(con, rid_ms, cid, ease, rtype, time_ms)`, `set_config(con, key, value)`, `ms(datetime) -> int`, `SEP`.
...
- Produces: `retention(days: DayRow[]): number | null`, `totals(days: DayRow[]): {reviews:number; minutes:number; newCards:number}`, `windowFrom(days: DayRow[], fromDate: string): DayRow[]`, `weekStart(todayKey: string): string`, `rankBy(people: PersonView[], pick: (p: PersonView) => number): PersonView[]`, `deckTotals(days: DayRow[]): [string, number][]`; and `GET(req: Request)`.
...
import { retention, totals, windowFrom, weekStart, rankBy, deckTotals } from "@/lib/metrics";
...
"use client";
import { retention, totals, rankBy, windowFrom, weekStart } from "@/lib/metrics";
import type { DayRow, PersonView } from "@/lib/types";
...
"use client";
import { deckTotals, retention, totals } from "@/lib/metrics";
import type { FeedItem, PersonView } from "@/lib/types";
